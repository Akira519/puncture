...
class ClientStateConnect(object):
    ...
    def _on_remote_write(self):
        self._stateControl._state = STAGE_STREAM
        if self._stateControl._data_to_write_to_remote:
            ...
            logging.debug('STAGE_CONNECTING try to write remotesock,From(Ip: %s,Port: %s) To (Remote IP: %s, port: %s)'
    ...
    def _on_local_read(self):
        ...
        if self._stateControl._state == STAGE_CONNECTING:
         #   logging.info('STAGE_CONNECTING _on_local_read , state: %s append data' % (self._stateControl._state))
            self._stateControl._data_to_write_to_remote.append(data)
            pass